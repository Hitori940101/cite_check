...
- **高峰期暂停开发 (CRITICAL)**：由于 LLM 提供商的计费策略，每日 **14:00–18:00** 为高峰时段（3 倍计费），其余时段为非高峰（1 倍计费）。在此项目上，高峰期内**禁止执行任何开发任务**（编码、测试、重构、API 调用等）。所有任务队列挂起，待 18:00 后自动恢复执行。如高峰期前已开始工作，应保存进度并提交当前变更后暂停。
