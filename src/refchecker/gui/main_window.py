...
Supports Chinese/English switching via i18n module.
"""
...
from refchecker.gui.i18n import t, set_language, get_language, toggle_language
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self) -> None:
        ...
        self._adapter_label.setStyleSheet("font-weight: bold;")
        adapter_row.addWidget(self._adapter_label)
    ...
    def _apply_language(self) -> None:
        """Update all UI text to match current language setting."""
        lang = get_language()
        self.setWindowTitle(t("app.title"))
        self._verify_btn.setText(t("btn.verify"))
        self._cancel_btn.setText(t("btn.cancel"))
        self._export_btn.setText(t("btn.export"))
        self._settings_btn.setText(t("btn.settings"))
        self._lang_btn.setText(t("btn.lang"))
        self._adapter_label.setText(t("label.adapters"))
        self._statusbar.showMessage(t("status.ready"))
        self._file_drop.update_language()
        self._result_table.update_language()
    ...
    def _on_file_selected(self, path: Path) -> None:
        ...
        try:
            ...
            self._statusbar.showMessage(t("status.loaded", count=count, file=path.name))
    ...
    def _start_verification(self) -> None:
        ...
        if not selected_indices:
            QMessageBox.warning(self, t("error.no_selection"), t("error.no_selection_msg"))
            return
        ...
        if not adapters:
            QMessageBox.warning(self, t("error.no_adapters"), t("error.no_adapters_msg"))
            return
        ...
        self._statusbar.showMessage(t("status.verifying"))
        self._worker.start()
    ...
    def _cancel_verification(self) -> None:
        ...
        self._cancel_btn.setEnabled(False)
        self._statusbar.showMessage(t("status.cancelled"))
    ...
    def _on_results_ready(self, results: list) -> None:
        ...
        total = len(results)
        self._statusbar.showMessage(t("status.done", verified=verified, total=total))

    def _on_error(self, error_msg: str) -> None:
        """Handle verification error."""
        self._verify_btn.setEnabled(bool(self._references))
        self._cancel_btn.setEnabled(False)
        self._statusbar.showMessage(t("status.error", msg=error_msg))
        QMessageBox.critical(self, t("error.verify_title"), error_msg)
    ...
    def _on_rate_limited(self, adapter_name: str) -> None:
        ...
        toast_mgr.show(
            t("toast.rate_limit", adapter=adapter_name),
            severity="warning",
            action_text=t("toast.open_settings"),
    ...
    def _show_settings(self) -> None:
        ...
        dialog.exec()
        # Re-apply language in case it was changed in settings
        self._apply_language()
    ...
    def _show_export_dialog(self) -> None:
        ...
        if not results:
            QMessageBox.information(self, t("export.no_results"), t("export.no_results_msg"))
            return
